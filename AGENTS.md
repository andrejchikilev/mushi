...
- Current implementation: domain schemas (Phase 1), filesystem storage (Phase 2), task/session/profile workflows with CLI (Phase 3), backend adapter protocol with Cursor CLI and OpenCode adapters and CLI integration (Phase 4), handoff generation and session resume (Phase 5). No search implementation yet.
...
6. **Document** — if everything is clean, create `.mushi-dev/manual-test-<phase>.md` with concrete shell commands for manual verification of that phase's changes. If not clean, do not create the guide and wait for instructions.
